...
// Función para escapar texto al guardar en CSV (comillas y comas)
...
// Función para leer un campo CSV que puede tener comillas y comas internas
        ...
        while (ss.get(c)) {
            if (c == '"') {
                if (ss.peek() == '"') { // comilla escapada
                    ss.get(); // consumir segunda comilla
                    resultado += '"';
                } else {
...
// Función para guardar movimientos de un cliente (usando escape)
void guardarMovimientosCSV(int codigo_cliente, const tMovimiento& movimientos_cliente) {
...
// Función para cargar movimientos de un cliente (usando lectura con soporte comillas)
void cargarMovimientosCSV(int codigo_cliente, tMovimiento& movimientos_cliente) {
    string nombre_archivo_mov = "movimientos_cliente_" + to_string(codigo_cliente) + ".csv";
...
void cargarClientesCSV(tContenedor& contenedor) {
    ...
    while (getline(archivo, linea) && contenedor.Lista.counterCliente < MAX_CLIENTES) {
        ...
        tCliente nuevo_cliente;
        string campo;

        getline(ss, campo, ','); nuevo_cliente.codigo = stoi(campo);
        getline(ss, campo, ','); nuevo_cliente.nombre = campo;
        getline(ss, campo, ','); nuevo_cliente.direccion = campo;
        getline(ss, campo, ','); nuevo_cliente.telefono = campo;
        getline(ss, campo, ','); nuevo_cliente.reservas = (campo == "1");

        // Carga los movimientos para este cliente
        cargarMovimientosCSV(nuevo_cliente.codigo, nuevo_cliente.movimiento);
        
        contenedor.Lista.clientes[contenedor.Lista.counterCliente] = nuevo_cliente;
        contenedor.Lista.counterCliente++;
    }
    archivo.close();
    cout << "Clientes cargados correctamente.\n";
}
...
void altaCliente(tContenedor& Contenedor) {
    ...
    cout << "Nombre: ";
    getline(cin, nuevo.nombre); // CORRECCIÓN: Uso directo de getline. El buffer ya está limpio si es la primera llamada
...
void mostrarEstadoCuenta(const tCliente& cliente) { 
    ...
    cout << string(50, '-') << "\n";  // En lugar de guiones fijos, repite 50 veces '-'
